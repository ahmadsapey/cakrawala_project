...
<body class="bg-slate-100 text-slate-800 font-sans antialiased selection:bg-indigo-500 selection:text-white flex items-center justify-center min-h-screen py-12 px-4 sm:px-6 lg:px-8">
    ...
    <div class="max-w-md w-full bg-white rounded-2xl p-8 sm:p-10 border-2 border-slate-200 shadow-xl relative">
        ...
        <div class="mb-8 space-y-1.5">
            <h1 class="text-2xl sm:text-3xl font-extrabold text-slate-900 tracking-tight">Buat Akun Baru</h1>
            <p class="text-slate-600 text-xs sm:text-sm leading-relaxed font-medium">Mulai perjalanan belajarmu bersama ribuan siswa cerdas lainnya.</p>
        </div>
        ...
        <form class="space-y-5" action="{{ route('siswa.register.submit') }}" method="POST">
            ...
            <div class="space-y-1.5">
                ...
                <div class="relative">
                    ...
                    <input type="password" name="password" required placeholder="Minimal 8 karakter" class="w-full pl-11 pr-12 py-3 bg-white border-2 border-slate-300 rounded-xl text-xs sm:text-sm text-slate-900 placeholder-slate-400 font-semibold focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-600 transition-all">
                </div>
            </div>
            ...
            <div class="grid gap-5 sm:grid-cols-2">
                <div class="space-y-1.5">
                    <label class="block text-xs font-bold uppercase tracking-wider text-slate-700">NISN</label>
                    <input type="text" name="nisn" value="{{ old('nisn') }}" required placeholder="Nomor induk siswa" class="w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-xs text-slate-900 placeholder-slate-400 focus:border-indigo-600 focus:bg-white focus:outline-none focus:ring-2 focus:ring-indigo-600/20">
                </div>
                <div class="space-y-1.5">
                    <label class="block text-xs font-bold uppercase tracking-wider text-slate-700">Kelas</label>
                    <input type="text" name="class_name" value="{{ old('class_name') }}" required placeholder="Contoh: XII IPA 1" class="w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-xs text-slate-900 placeholder-slate-400 focus:border-indigo-600 focus:bg-white focus:outline-none focus:ring-2 focus:ring-indigo-600/20">
                </div>
            </div>
