...
<body class="min-h-screen bg-slate-100 pb-28 font-sans text-slate-800 antialiased">
    ...
    <main class="mx-auto max-w-3xl space-y-6 px-4 py-8 sm:px-6 lg:px-12">
        ...
        <form method="POST" action="{{ route('guru.material.store') }}" enctype="multipart/form-data" class="space-y-5 rounded-2xl border-2 border-slate-300 bg-white p-6 shadow-sm sm:p-8">
            @csrf
            ...
            <label class="block text-xs font-extrabold uppercase tracking-wider text-slate-700">
                Mata Pelajaran
                <input name="subject" value="{{ old('subject') }}" required placeholder="Contoh: Fisika Modern" class="mt-2 w-full rounded-xl border-2 border-slate-300 bg-white px-4 py-3 text-xs sm:text-sm font-semibold text-slate-900 focus:border-indigo-600 focus:ring-2 focus:ring-indigo-500/20 focus:outline-none transition-all">
            </label>

            <label class="block text-xs font-extrabold uppercase tracking-wider text-slate-700">
                Judul Materi
                <input name="title" value="{{ old('title') }}" required placeholder="Contoh: Eksperimen Efek Fotolistrik" class="mt-2 w-full rounded-xl border-2 border-slate-300 bg-white px-4 py-3 text-xs sm:text-sm font-semibold text-slate-900 focus:border-indigo-600 focus:ring-2 focus:ring-indigo-500/20 focus:outline-none transition-all">
            </label>

            <label class="block text-xs font-extrabold uppercase tracking-wider text-slate-700">
                Ringkasan Ajar
                <textarea name="summary" rows="4" placeholder="Penjelasan singkat materi untuk dibaca siswa..." class="mt-2 w-full rounded-xl border-2 border-slate-300 bg-white px-4 py-3 text-xs sm:text-sm font-semibold text-slate-900 focus:border-indigo-600 focus:ring-2 focus:ring-indigo-500/20 focus:outline-none transition-all resize-none">{{ old('summary') }}</textarea>
            </label>

            <label class="block text-xs font-extrabold uppercase tracking-wider text-slate-700">
                Video Rekomendasi (URL YouTube)
                <input type="url" name="video_url" value="{{ old('video_url') }}" placeholder="https://youtube.com/..." class="mt-2 w-full rounded-xl border-2 border-slate-300 bg-white px-4 py-3 text-xs sm:text-sm font-semibold text-slate-900 focus:border-indigo-600 focus:ring-2 focus:ring-indigo-500/20 focus:outline-none transition-all">
            </label>
            ...
            <div class="flex gap-3 pt-3">
                ...
                <button type="submit" name="status" value="published" class="flex-1 rounded-xl bg-indigo-600 border border-indigo-700 px-4 py-3 text-center text-xs font-extrabold text-white shadow-md hover:bg-indigo-700 transition-all">Terbitkan Sekarang</button>
            </div>
        </form>
    </main>
