    ...
    <div class="mx-auto flex h-16 max-w-7xl items-center justify-between px-4 sm:px-6">
        ...
        <nav class="ml-auto flex items-center gap-1" aria-label="Navigasi guru">
            ...
            <details class="group relative">
                <summary class="inline-flex cursor-pointer list-none items-center gap-2 rounded-xl {{ request()->routeIs('guru.material.*') || request()->routeIs('guru.tugas.*') ? 'bg-indigo-50 text-indigo-600' : 'text-slate-500' }} px-4 py-2.5 text-sm font-medium transition-colors hover:bg-indigo-50 hover:text-indigo-600">
                ...
                <div class="absolute right-0 mt-2 w-52 rounded-2xl border border-slate-100 bg-white p-2 shadow-xl shadow-indigo-100/50">
                    <a href="{{ route('guru.material.create') }}" class="flex items-center justify-between rounded-xl px-3 py-2.5 text-xs font-semibold text-slate-600 transition-colors hover:bg-indigo-50 hover:text-indigo-600"><span>Upload Materi</span><span class="text-[10px] text-slate-400">Modul</span></a>
            ...
            <a href="{{ route('guru.koreksi.tugas') }}" class="inline-flex items-center gap-2 rounded-xl {{ request()->routeIs('guru.koreksi.*') ? 'bg-indigo-50 text-indigo-600' : 'text-slate-500' }} px-4 py-2.5 text-sm font-medium transition-colors hover:bg-indigo-50 hover:text-indigo-600">
