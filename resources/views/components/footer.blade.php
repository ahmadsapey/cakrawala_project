@php($footerContent = $footerContent ?? null)
...
<footer id="contact" class="bg-[#080B13] text-slate-400 text-xs py-16 border-t border-slate-900">
    <div class="max-w-7xl mx-auto px-6 sm:px-8 lg:px-12">
        <div class="grid grid-cols-1 md:grid-cols-4 gap-10 pb-12 border-b border-slate-800/80">
            <div class="space-y-4">
                <h4 class="font-bold text-white text-sm">{{ $footerContent?->title ?? 'Cakrawala PT. INDO PRESTASI UTAMA' }}</h4>
            ...
            <div class="space-y-3">
                ...
                <ul class="space-y-2 text-xs">
                        <li><a href="mailto:{{ $footerContent?->meta ?? 'info@cakrawala.id' }}" class="hover:text-white transition-colors">{{ $footerContent?->meta ?? 'info@cakrawala.id' }}</a></li>
                        <li><a href="tel:{{ $footerContent?->cta_label ?? '+6281234567890' }}" class="hover:text-white transition-colors">{{ $footerContent?->cta_label ?? '+62 812-3456-7890' }}</a></li>
                </ul>
            </div>
        </div>
