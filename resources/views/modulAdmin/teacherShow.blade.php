...
<head>
    ...
    <title>Biodata {{ $teacher->user?->name }} | Cakrawala Educentre</title>
    <!-- Tailwind CSS CDN -->
...
<body class="min-h-screen bg-gradient-to-br from-indigo-50/50 via-slate-50 to-blue-50/40 pb-32 font-sans text-slate-800 antialiased selection:bg-indigo-500 selection:text-white">
    ...
    <main class="mx-auto flex w-full max-w-3xl flex-col space-y-6 px-4 py-8 sm:px-6 lg:px-12">
        ...
        <div class="flex items-center justify-between gap-4 rounded-3xl border-2 border-indigo-200 bg-white/90 backdrop-blur-md p-6 sm:p-8 shadow-sm">
            <div>
                <span class="text-xs font-black uppercase tracking-[0.18em] text-indigo-700 bg-indigo-50 border border-indigo-200 px-3 py-1.5 rounded-xl inline-block shadow-2xs">Biodata Guru</span>
                <h1 class="mt-2 text-2xl sm:text-3xl font-black text-slate-900 tracking-tight">{{ $teacher->user?->name }}</h1>
            </div>
        ...
        <div class="grid gap-6 rounded-3xl border-2 border-indigo-200 bg-white/90 backdrop-blur-md p-6 sm:p-8 shadow-sm sm:grid-cols-2">
            <div>
                <p class="text-xs font-black uppercase tracking-wider text-slate-400">NIP / Identitas Tutor</p>
                <p class="mt-1 font-bold text-slate-900">{{ $teacher->nip }}</p>
            </div>
            <div>
                <p class="text-xs font-black uppercase tracking-wider text-slate-400">Mata Pelajaran</p>
                <p class="mt-1 font-bold text-slate-900">{{ $teacher->subject }}</p>
            </div>
            <div>
                <p class="text-xs font-black uppercase tracking-wider text-slate-400">Email Akun</p>
                <p class="mt-1 font-bold text-slate-900">{{ $teacher->user?->email }}</p>
            </div>
            <div>
                <p class="text-xs font-black uppercase tracking-wider text-slate-400">No. WhatsApp / HP</p>
                <p class="mt-1 font-bold text-slate-900">{{ $teacher->phone ?: '-' }}</p>
            </div>
            <div>
                <p class="text-xs font-black uppercase tracking-wider text-slate-400">Terdaftar Sejak</p>
                <p class="mt-1 font-bold text-slate-900">{{ $teacher->created_at?->format('d M Y') ?: '-' }}</p>
            </div>
            <div>
                <p class="text-xs font-black uppercase tracking-wider text-slate-400">Status</p>
                <div class="mt-1">
                    <span class="px-3 py-1.5 {{ $teacher->status === 'active' ? 'bg-emerald-50 border-emerald-300 text-emerald-800' : 'bg-rose-50 border-rose-300 text-rose-800' }} border text-xs font-black rounded-xl inline-block">
