    ...
    <div
        ...
        <div class="flex flex-col justify-between gap-4 pt-2 sm:flex-row sm:items-end">
            <div>
                ...
                <p class="text-xs text-slate-500 mt-1">Kelola data siswa, NISN, dan rombel kelas aktif.</p>
            </div>
        ...
        <div class="flex items-center space-x-2 overflow-x-auto no-scrollbar">
            ...
            <a href="{{ route('admin.siswa.index', array_filter(['status' => 'active', 'search' => request('search')])) }}"
            ...
            <a href="{{ route('admin.siswa.index', array_filter(['status' => 'inactive', 'search' => request('search')])) }}"
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div class="flex items-center space-x-3.5 min-w-0">
                            ...
                            <div class="space-y-0.5 truncate">
                                <h3 class="text-xs sm:text-sm font-extrabold text-slate-900 truncate">
                                    {{ $student->user?->name ?? 'Siswa' }}</h3>
                                <p class="text-xs text-slate-600 font-semibold truncate">NISN: {{ $student->nisn ?? '-' }} •
                                    {{ $student->class_name ?? 'Belum ada kelas' }}</p>
                            </div>
                        </div>
                        <span
                            class="px-3 py-1 {{ $student->status === 'active' ? 'bg-emerald-100 border-emerald-300 text-emerald-800' : 'bg-rose-100 border-rose-300 text-rose-800' }} border text-xs font-extrabold rounded-lg flex-shrink-0">
                            {{ $student->status === 'active' ? 'Aktif' : 'Tidak Aktif' }}
                        </span>
                    </div>

                    <div class="flex items-center justify-between border-t border-slate-100 pt-3 text-xs">
                        <span class="text-slate-400 font-medium truncate">{{ $student->user?->email }}</span>
            ...
            @empty
                <div
                    ...
                    Tidak ada data siswa yang sesuai kriteria.
                </div>
            @endforelse
        </div>
